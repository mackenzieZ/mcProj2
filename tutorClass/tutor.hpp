...
Tutor(string tutorName, bool cleared, shift schedule[NUM_DAYS], map<string,bool> m):
...
Tutor(string tutorName, bool cleared, shift schedule[NUM_DAYS]):
m_name(tutorName), m_cleared(cleared)
{
    defaultAbilities();
}
Tutor(string tutorName, bool cleared):
m_name(tutorName), m_cleared(cleared)
{
    defaultSchedule();
    defaultAbilities();
...
void changeAbilities(string key, bool newValue){
